...
def player_stats(
    ...
    if selected_side in {"CT", "T"}:
        ...
        active_sides = {"CT", "T"}
    game_rounds_count = 0  # Xaxanalytcis - Variable to get the actual round number
    totalDamage_temp = 0
    for r in game_rounds:
        ...
        for side in [team.lower() + "Side" for team in active_sides]:
            ...
            for p in r[side]["players"] or []:
                player_key = p["playerName"] if p["steamID"] == 0 else str(
                    p["steamID"])
                if player_key not in player_statistics:
                    player_statistics[player_key] = {
                        ...
                        "deaths": 0,
                        "kdr": 0,
                        "assists_awpy": 0,  # Default awpy calculation of assists
                        "assists": 0,  # Custom - We defined another calculation for the assists
                        "tradeKills": 0,
                        "tradedDeaths": 0,
                        "teamKills": 0,
                        "suicides": 0,
        ...
        for k in r["kills"] or []:
            ...
            if k["victimSide"] in players_killed:
                players_killed[k["victimSide"]].add(
                    victim_key)  # type: ignore[index]
            ...
            if (
                ...
                and len(r[k["victimSide"].lower() + "Side"]["players"] or [])
                # type: ignore[literal-required, index]
                ...
                for player in r[k["victimSide"].lower() + "Side"]["players"]:
                    ...
                    if (
                        ...
                        enemies_alive = len(
                            r[other_side(k["victimSide"]).lower() +
                              "Side"]["players"] or []  # type: ignore
                        ) - len(
                            # type: ignore
                            players_killed[other_side(k["victimSide"])]
                        )
                        if enemies_alive > 0:
                            player_statistics[clutcher_key][
                                # type: ignore[literal-required]
                                f"attempts1v{enemies_alive}"
                            ] += 1
                            if r["winningSide"] == k["victimSide"]:
                                player_statistics[clutcher_key][
                                    # type: ignore[literal-required]
                                    f"success1v{enemies_alive}"
                                ] += 1
            if k["isTrade"]:
            ...
            if (
                ...
                player_statistics[assister_key]["assists_awpy"] += 1
                kast[assister_key]["a"] = True

                ###############################################################
                #                Xaxanalytics custom code block               #
                ###############################################################
                # Rule: We added a iteration into the damage object           #
                # to count as assistance only when the attacker have          #
                # sum of the damage higher than 40                            #
                #                                                             #
                ###############################################################

                # Logging awpy method to calculate the assistance
                # print("Assists logs awpy method | Round: "
                #       + str(game_rounds_count)
                #       + " - "
                #       + str(player_statistics[assister_key]["playerName"])
                #       + " assisted "
                #       + str(k["attackerName"])
                #       + " to kill "
                #       + str(k["victimName"]))
                ...
                for d in r["damages"] or []:
                    ...
                    if (attacker_key_temp_dmg == assister_key
                        and victim_key_temp == victim_key
                        and d["attackerSide"] in active_sides
                        and attacker_key_temp_dmg in player_statistics
                        and d["attackerSteamID"]
                            and not d["isFriendlyFire"]):
                        totalDamage_temp += d["hpDamageTaken"]

                if totalDamage_temp >= 40:
                    # Logging Xaxanalytics method to calculate the assistance
                    # print("Assists logs Xaxanalytics method | Round: "
                    #       + str(game_rounds_count)
                    #       + " - "
                    #       + "|Assister: "
                    #       + str(player_statistics[assister_key]["playerName"])
        ...
        for w in r["weaponFires"] or []:
            fire_key = (
                w["playerName"] if w["playerSteamID"] == 0 else str(
                    w["playerSteamID"])
            )
        ...
        for b in r["bombEvents"] or []:
            player_key = (
                b["playerName"] if b["playerSteamID"] == 0 else str(
                    b["playerSteamID"])
            )
        ...
        for player, n_kills in round_kills.items():
            if n_kills in range(6):  # 0, 1, 2, 3, 4, 5
                # type: ignore[literal-required]
                player_statistics[player][f"kills{n_kills}"] += 1

        # Xaxanalytics - advancing with the round count
        game_rounds_count = game_rounds_count+1
